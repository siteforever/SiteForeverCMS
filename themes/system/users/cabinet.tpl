...
{else}
    ...
    {if $user.perm != $smarty.const.USER_WHOLE}
        <p>Если Вы представляете организацию и желаете иметь аккаунт оптового 
        покупателя, Вам необходимо заполнить все данные в <a {href url="users/edit"}>профиле</a> 
        и <a {href url="users/option=whole_request"}>отправить нам заявку</a> на изменение статуса аккаунта.</p>
    {/if}
