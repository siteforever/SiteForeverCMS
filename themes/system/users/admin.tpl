{form class="well form-horizontal" action="users/admin" method="get"}
    ...
    {if $request->get('search')}{a controller="users" action="admin" class="btn"}Сбросить фильтр{/a}{/if}
    </div>
{/form}
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                {a controller="users" action="adminEdit" id=$user.id class="edit" title=t('Edit')}{$user.login}{/a}
            </td>
            <td>{$user.email}</td>
            <td>{$user.lname}</td>
            <td>{$user.phone}</td>
