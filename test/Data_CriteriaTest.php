...
class Data_CriteriaTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testParamArray()
    {
        ...
        $criteria = new Data_Criteria('test',array(
            'cond'  => '`id` IN (?)',
            'params'=> array(array(0,1,2,3)),
        ));

        $this->assertEquals(
            'SELECT * FROM `test` WHERE `id` IN (0,1,2,3)',
            $criteria->getSQL()
        );
        ...
        $this->assertEquals(
            'SELECT * FROM `test` WHERE `id` IN (\'a\',\'b\',\'c\',\'d\')',
            $criteria->getSQL()
        );
    }
    ...
    {
        $criteria = new Data_Criteria('structure', array(
            'cond'      => '`deleted` = ?',
            'params'    => array(0),
            'order'     => '`pos`',
        ));
        ...
        $this->assertEquals(
            'SELECT * FROM `structure` WHERE `deleted` = 0 ORDER BY `pos`',
            $criteria->getSQL()
        );
    }
}
