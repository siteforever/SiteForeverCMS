...
if (!empty($_POST['login']) && isset($_POST['pass'])) {
	if (@mysql_connect(DBHOST, $_POST['login'], $_POST['pass'])){
		setcookie("sxd", base64_encode("SKD101:{$_POST['login']}:{$_POST['pass']}"), 84600, '/');
		//header("Location: /misc/dumper/dumper.php");
        print "<script>window.location.href = 'dumper.php';</script>";
		mysql_close();
		exit;
	}
	else{
