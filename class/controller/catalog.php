...
class Controller_Catalog extends Controller
{
    ...
    protected function indexCategories( Data_Object_Catalog $item )
    {
        ...
        $paging = $this->paging(
            $count, 10, $this->router->createLink( $this->page[ 'alias' ], array( 'cat'=> $item->getId() ) )
        );
        ...
        if ($from_string = @unserialize( $path )) {
            if ($from_string && is_array( $from_string )) {
                foreach ( $from_string as $val ) {
                    $bc[ ] = '<a href="' . $this->router->createServiceLink(
                        'catalog', 'admin', array( 'part'=> $val[ 'id' ] )
                    ) . '">' . $val[ 'name' ] . '</a>'
                        . '<a href="' . $this->router->createServiceLink(
                        'catalog', 'category', array( 'edit'=> $val[ 'id' ] )
                    ...
                    if ($xml) {
                        ...
                        foreach ( $xml_data as $trade )
                        {
                            ...
                            $upd_data[ ] = array(
                                ...
                                'price1'    => isset( $trade[ 'price1' ] ) ? number_format(
                                    $trade[ 'price1' ], 2, '.', ''
                                ) : '0.00',
                                'price2'    => isset( $trade[ 'price2' ] ) ? number_format(
                                    $trade[ 'price2' ], 2, '.', ''
                                ) : '0.00',
                            ...
                            $html[ ] = "<tr><td>{$trade->txtart}</td><td>{$trade->txtname}</td>" .
                                "<td>" . ( $trade->price1 ? "{$trade->price1}&nbsp;{$trade->item1}/{$trade->currency1}"
                                : "&mdash;" ) . "</td>" .
                                "<td>" . ( $trade->price2 ? "{$trade->price2}&nbsp;{$trade->item2}/{$trade->currency2}"
                                : "&mdash;" ) . "</td>" .
